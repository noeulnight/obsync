...
목표: 같은 Nest 앱에서 폐기 가능한 MCP 연결을 제공한다.

- [ ] MCP SDK를 추가한다.
- [ ] Streamable HTTP `/mcp` endpoint를 추가한다.
- [ ] `McpToken` model과 migration을 추가한다.
- [ ] token 생성·목록·폐기 endpoint를 추가한다.
- [ ] MCP token hash만 저장한다.
- [ ] MCP request에서 사용자를 복원한다.
- [ ] 인증 성공·실패 E2E를 추가한다.

완료 게이트:

- [ ] 폐기된 MCP token은 즉시 사용할 수 없다.
- [ ] MCP token 원문은 생성 응답에서 한 번만 보인다.
- [ ] REST/WebSocket과 동일한 사용자 경계를 가진다.
...
- [ ] Vault 목록 tool을 추가한다.
- [ ] manifest 기반 파일 목록 tool을 추가한다.
- [ ] Markdown 읽기 tool을 추가한다.
- [ ] Markdown 쓰기 tool을 Yjs update로 구현한다.
- [ ] 모든 tool에 Vault owner 검사를 적용한다.
- [ ] tool 호출 audit log를 추가한다.
...
- [ ] audit log에 token과 문서 본문이 기록되지 않는다.
