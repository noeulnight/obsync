...
| 18 | MCP 인증과 transport | 17 | [x] |
| 19 | MCP Vault/Markdown tools | 18 | [x] |
| 20 | 운영 안정화 | 19 | [ ] |
...
- [x] Web Account 설정에서 연결 앱을 조회하고 즉시 철회한다.
...
- [x] 파일 생성·이름 변경·삭제 tool을 추가한다.
- [x] Canvas 읽기·쓰기 tool을 추가한다.
- [x] 첨부파일 upload/download tool을 추가한다.
- [x] 백링크·그래프·버전 기록 tool을 추가한다.
- [x] 모든 tool에 Vault 권한 검사를 적용한다.
- [x] MCP 쓰기 → Hocuspocus client 반영 E2E를 추가한다.

완료 게이트:

- [x] 다른 사용자의 Vault를 tool로 읽거나 쓸 수 없다.
- [x] Markdown 쓰기가 열린 Obsidian에 실시간 전달된다.
